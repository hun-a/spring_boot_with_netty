...
public class EmployeeRepository {
    ...
    public Mono<String> save(Employee employee) {
        if (isExists(employee)) {
            return Mono.empty();
        }

        employeeData.put(employee.getId(), employee);
        return Mono.just(employee.getId());
    }

    private boolean isExists(Employee employee) {
        return !ObjectUtils.isEmpty(employeeData.get(employee.getId()));
    }
